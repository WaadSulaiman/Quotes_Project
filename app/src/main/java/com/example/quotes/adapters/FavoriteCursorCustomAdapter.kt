...
class FavoriteCursorCustomAdapter(context: Context, cursor: Cursor) :
    ...
    override fun bindView(view: View?, context: Context?, cursor: Cursor?) {
        ...
        if (cursor!!.getString(2).isEmpty()) {
            quoteTextAuthor?.setText(R.string.by_unknown)
        } else {
        ...
        cardView?.setOnLongClickListener {
            ...
            val builder = AlertDialog.Builder(view.context).setTitle("Delete").setMessage(
                ...
                swapCursor(SampleSQLiteDBHelper(view.context).readFromDB())
                dialog.dismiss()
